...
void Registro(){
	...
	if (log == NULL)
	{
		...
		gets(lg.password);
		fflush(stdin);
		//se guardan el usuario y contraseña en cuenta.db
...
void InicioSesion(){
	...
	int Confirm=0;//una confirmacion que indica que el usuario y contraseña son correctos
...
//se piden credenciales de inicio de sesion
	...
	{//usuario y contraseña se separan en 2 tokens
		...
		strcpy(p,val2);
		removerEspaciosblanco(p);//se remueven posibles espacios de la contraseña
		removerEspaciosblanco(password);
		if(strcmp(p,password)==0){//se pregunta si las contraseñas coinciden
